...
public interface FileMapper {
    ...
    List<FileEntity> getFilesByFolderId(String folderId);
    ...
    void updateFileName(@Param("id") String id, @Param("fileName") String fileName);

    @Update("UPDATE file SET parentFolderId = #{newFolderId} WHERE id = #{fileId}")
    void moveFile(@Param("fileId") String fileId, @Param("newFolderId") String newFolderId);

    @Update("UPDATE file SET isDelete = 'Y' WHERE id = #{fileId}")
    void recycleFile(String fileId);

    @Select("SELECT * FROM file WHERE id = #{id}")
    Map<String, Object> getFileInfoById(String id);

    @Select("SELECT * FROM file WHERE createBy = #{ownerId} AND parent_folder_id = #{folderId}")
    List<Map<String, Object>> shareFile(@Param("ownerId") String ownerId, @Param("folderId") String folderId);
